...
class Game:
    ...
    def __init__(self, expectUserInputs=True, ai_player=2):
    ...
    # Each call simula um passo do mundo
    def update(self):  
        ...
        # Switch player turn (determine who moves now)
        ...
        # If it's AI's turn and AI enabled, compute its move and enqueue it
        if self.ai_player is not None and gs.currentPlayer == self.ai_player:
            move = self.oracle.find_best_move(gs.grid, gs.currentPlayer)
            if move and move != (-1, -1):
                self.eventJournal.append(move)
        
        # If there is no event to process, do nothing
    ...
    def get_oracle_move(self):
        ...
        if gs.currentPlayer == 0:
            ...
            next_player = 1
        return self.oracle.find_best_move(gs.grid, next_player)
